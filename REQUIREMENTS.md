...
- **C-14b** On the lock screen / OS media notification, there's no way for the app to ask the OS/launcher how many lines its widget actually has room for — it varies by device and can't be detected from a webpage. So rather than gamble on a field going unseen, both of `MediaMetadata`'s guaranteed fields carry two pieces of info each: `title` is the station name alone, or "station -- program" once a program is known — the station itself is never fully displaced, only ever gains a program name alongside it; `artist` is "artist - title" track info, or "Live radio" when nothing's known. (`album` is unused — everything already fits in the two fields every view shows.)
