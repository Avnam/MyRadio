...
- **A `nowPlaying` entry is a static config, not a live subscription.** The song/program it reports updates live (polling while playing) — but *which endpoint and field paths to read* only changes when someone edits `directory/<CODE>_metadata.json` by hand and it gets deployed. If a station changes its API shape, the entry doesn't self-heal.
