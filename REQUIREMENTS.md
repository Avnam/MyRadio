...
- **C-13a** Once a station is loaded and a connection attempt is in flight, a rapid next/previous/play press never interrupts it — the request is queued (only the latest survives a burst) and applied as soon as the current attempt settles, or after 900ms if it's taking unusually long. Two compounding bugs used to break this: (1) every press tore down the in-flight `<audio>` element immediately, killing a connection before it had any real chance to succeed — real streams often take longer than a few hundred ms; (2) `playing` only became true once audio was actually flowing, not from the moment an attempt started, so a press made while the station you'd just switched to was still buffering read `playing` as false and silently downgraded to "select only" (decision #2) — no error, no queue, just quietly dropped.
...
- **Song titles (C-14), tested in a real browser:** all 7 stations do interleave ICY song-title metadata in their stream (`icy-metaint: 16000` on every one), but **none currently let the browser read it.** Reading it requires a second `fetch()` connection with an `Icy-MetaData` request header, and every one of the 7 fails that request's CORS preflight — confirmed via a real Edge session, not just header inspection (three of them look permissive to a plain `curl`, but the browser's preflight is stricter and rejects the custom header). So today, song titles for these 7 stations need the server-side fallback already anticipated under "Later" — none of them can be done client-side as-is.
...
- **Song titles via a server**, for stations the browser can't read directly — which, per this round of testing, is currently all 7 test stations. The server's job would be to hold the ICY connection open and expose just the parsed title (not proxy the whole audio stream).
