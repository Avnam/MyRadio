...
- **C-14b** What the lock screen / OS media notification actually says is a theme's presentation decision, the same category as the in-app header — not a core concern. A theme opts in with `getMediaMetadata(station, nowPlaying)`, returning `{title, artist}`; core's own fallback for a theme that doesn't define one (`app.js`'s `defaultMediaMetadata`) is deliberately plain: just the station name and the program name or "Live radio", not opinionated about any particular theme's UX. See E-9 for Evia's own choice.
...
- **E-9** Evia's lock screen wording (C-14b): there's no way to ask the OS/launcher how many lines its widget actually has room for — it varies by device and can't be detected from a webpage. So rather than gamble a field goes unseen, both of `MediaMetadata`'s guaranteed-visible fields carry two pieces of info each: `title` is the station name alone, or "station -- program" once a program is known — the station is never fully displaced, only ever gains company; `artist` is "artist - title" track info, or "Live radio" when nothing's known. A different theme is free to word this however it wants, or not define it at all and get core's plain station-name-only fallback.
